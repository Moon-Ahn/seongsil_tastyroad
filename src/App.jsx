  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 🌟 [개선된 그룹화 로직] 여러 줄 중 하나라도 'o'가 있으면 인정함
        const grouped = data.reduce((acc, curr) => {
          const resName = curr.restaurant;
          // 'o' 표시 확인 (공백 제거, 소문자화하여 더 정확하게 체크)
          const isLunchMarked = curr.isLunch && curr.isLunch.toString().trim().toLowerCase() === 'o';
          const isVisitMarked = curr.isVisitation && curr.isVisitation.toString().trim().toLowerCase() === 'o';

          if (!acc[resName]) {
            acc[resName] = {
              id: resName, name: resName, address: curr.address || "",
              category: curr.category || "기타",
              isLunch: isLunchMarked,
              isVisitation: isVisitMarked,
              reviews: [], avgRating: 0
            };
          } else {
            // 이미 있는 식당이라면, 'o' 표시가 새로 발견되었을 때 업데이트
            if (isLunchMarked) acc[resName].isLunch = true;
            if (isVisitMarked) acc[resName].isVisitation = true;
          }
              ...
              {!isManualAddress ? (
                <div className="bg-slate-50 p-5 rounded-[28px] border border-slate-100 space-y-4 shadow-inner">
                  <div className="flex gap-2">
                    <input type="text" className="flex-1 p-3.5 bg-white rounded-2xl shadow-sm outline-none text-base lg:text-sm focus:ring-2 focus:ring-orange-400" placeholder="예: 수유리 우동집" value={searchKeyword} onChange={e => setSearchKeyword(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), searchPlaces())} />
