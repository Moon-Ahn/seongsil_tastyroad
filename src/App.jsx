...
// 🌟 2. 팀장님의 카카오 JavaScript 키 (index.html에서 사용 중이므로 여기서는 참조용)
const KAKAO_JS_KEY = "543a2abb508497df4a86c32887514698";
...
const App = () => {
  ...
  const onTouchStart = (e) => {
    ...
    // 핸들을 잡았거나 시트가 닫혀있을 때 드래그 시작
    if (isHandle || sheetY > 70) isSheetDraggingRef.current = true;
    else isSheetDraggingRef.current = false;
  };
  ...
  const onTouchMove = (e) => {
    ...
    // 리스트가 맨 위이고 아래로 내릴 때 혹은 드래그 모드일 때 시트 이동
    if (isSheetDraggingRef.current || (scrollTop <= 0 && deltaY > 0)) {
  ...
  const onTouchEnd = () => {
    ...
    if (sheetY < snapThreshold) setSheetY(60); // 펼침
    else {
      setSheetY(window.innerHeight - peekHeight); // 접음
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        myLocMarkerRef.current = new window.google.maps.Marker({
          ...
          icon: { path: window.google.maps.SymbolPath.CIRCLE, fillColor: '#3B82F6', fillOpacity: 1, strokeWeight: 2, strokeColor: '#FFFFFF', scale: 8 }
        });
      },
  ...
  return (
    <div className="h-screen bg-slate-50 font-sans text-slate-900 flex flex-col overflow-hidden">
      ...
      <header className="fixed top-0 left-0 w-full z-30 lg:static bg-white/95 border-b border-slate-200 px-6 py-4 flex justify-between items-center shadow-md lg:shadow-none">
        ...
        <button onClick={() => setIsAddModalOpen(true)} className="bg-slate-900 text-white px-4 py-2 rounded-full text-xs lg:text-sm font-bold flex items-center gap-1.5 shadow-md">
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden relative pt-[73px] lg:pt-0">
        ...
        {/* --- 🗺️ 지도 구역 --- */}
        <section className="absolute inset-0 z-0 lg:static lg:w-1/2 lg:h-full bg-slate-200" style={{ touchAction: isMobile ? 'none' : 'auto' }}>
          <div ref={mapRef} className="w-full h-full" />

          {/* 🌟 내 위치 버튼 */}
          <button
            onClick={moveToMyLocation}
            className="absolute right-4 bottom-[180px] lg:bottom-6 z-10 bg-white p-3 rounded-full shadow-xl border border-slate-200 text-slate-700 active:scale-95 transition-all"
          >
            <Navigation size={24} className="fill-slate-700" />
          </button>
        ...
        {/* --- 📝 바텀 시트 (리스트/상세) --- */}
        <section
          className="fixed bottom-0 left-0 w-full z-20 bg-white rounded-t-3xl lg:rounded-none shadow-2xl flex flex-col lg:static lg:w-1/2 lg:h-full lg:shadow-none lg:border-l lg:border-slate-200 overflow-hidden"
          style={mobileSheetStyle} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}
        >
          {/* 핸들 */}
          <div className="sheet-handle-area lg:hidden w-full p-4 flex justify-center items-center cursor-ns-resize shrink-0">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-100 shrink-0 bg-slate-50">
              <div className="relative">
                ...
                <input type="text" placeholder="맛집 이름이나 주소 검색..." className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-orange-400 text-base lg:text-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
              <div className="flex gap-2 mt-3 overflow-x-auto custom-scrollbar pb-1">
                {CATEGORIES.map(cat => <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap shadow-sm border ${activeCategory === cat ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100'}`}>{cat}</button>)}
              </div>
            </div>

            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
              {/* 리스트 목록 */}
              <div ref={listRef} className={`w-full lg:w-2/5 shrink-0 overflow-y-auto custom-scrollbar p-2 space-y-2 bg-slate-50/50 transition-all ${selectedRes && isMobile ? 'invisible opacity-0' : 'visible opacity-100'}`} style={{ overscrollBehavior: 'contain' }}>
                {activeCategory === '전체' && (
                  <button onClick={() => setSelectedRes(null)} className="w-full text-left p-4 rounded-xl border-2 bg-blue-50 border-blue-400 mb-2">
                    <h3 className="font-bold text-blue-900 text-sm">수유 성실교회</h3><p className="text-[11px] text-blue-500 mt-1">우리들의 베이스캠프</p>
                  </button>
                )}
                {filteredList.map(res => (
                  <div key={res.id} onClick={() => setSelectedRes(res)} className={`p-4 rounded-xl cursor-pointer border-2 transition ${selectedRes?.id === res.id ? 'bg-orange-50 border-orange-400' : 'bg-white border-transparent shadow-sm'}`}>
                    ...
                    <div className="flex items-center gap-1 text-xs font-bold text-slate-600">
                      ...
                      {/* 🌟 교회에서의 거리 명시 */}
                      {distances[res.id] && (
                        <span className="ml-1 text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
                ...
                {selectedRes && (
                  <>
                    <div className="flex justify-between items-start mb-6 pb-4 border-b">
                      <div>
                        <span className="text-xs font-bold text-orange-600 bg-orange-100 px-2 py-1 rounded">{selectedRes.category}</span>
                        <h2 className="text-2xl font-black text-slate-900 mt-2">{selectedRes.name}</h2>
                        <div className="flex flex-col gap-1 mt-2">
                           <p className="text-xs text-slate-500 flex items-center gap-1"><MapPin size={12}/>{selectedRes.address}</p>
                           {/* 🌟 교회 거리 상세 명시 */}
                           {distances[selectedRes.id] && (
                             <p className="text-xs font-bold text-blue-600 flex items-center gap-1">
                               <Church size={12} /> 성실교회에서 {Math.round(distances[selectedRes.id])}m 거리에 있어요
                             </p>
                           )}
                        </div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => setIsReviewModalOpen(true)} className="bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-bold">리뷰 쓰기</button>
                      </div>
                    </div>
                      ...
                      {selectedRes.reviews.map((r, i) => (
                        <div key={i} className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <div className="flex">{[1,2,3,4,5].map(num => <Star key={num} size={12} className={num <= r.rating ? "fill-orange-400 text-orange-400" : "text-slate-200"} />)}</div>
                          </div>
                          <p className="text-sm text-slate-700 leading-relaxed">{r.comment}</p>
                          <p className="text-[10px] text-slate-400 mt-2">{new Date(r.timestamp).toLocaleDateString()}</p>
                        </div>
                      ))}
                    </div>
                  </>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <form onSubmit={handleAddRestaurant} className="bg-white rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center pb-2 border-b"><h2 className="text-xl font-black">새 맛집 제보하기</h2><X className="cursor-pointer text-slate-400" onClick={() => setIsAddModalOpen(false)} /></div>
            <div className="overflow-y-auto flex-1 space-y-4 py-2 custom-scrollbar">
              <div className="flex justify-end"><label className="flex items-center gap-2 text-xs font-medium text-slate-500 cursor-pointer"><input type="checkbox" className="w-4 h-4" checked={isManualAddress} onChange={(e) => setIsManualAddress(e.target.checked)} />직접 주소 입력</label></div>
              {!isManualAddress ? (
                <div className="bg-slate-50 p-4 rounded-2xl border space-y-3">
                  <div className="flex gap-2">
                    <input type="text" className="flex-1 p-3 bg-white rounded-xl border outline-none text-base lg:text-sm" placeholder="식당 이름 검색" value={searchKeyword} onChange={e => setSearchKeyword(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), searchPlaces())} />
                    <button type="button" onClick={searchPlaces} className="px-4 py-3 bg-slate-800 text-white rounded-xl font-bold text-xs">{isSearching ? <Loader2 className="animate-spin" size={16}/> : '검색'}</button>
                  </div>
                  {searchResults.length > 0 && (
                    <ul className="max-h-40 overflow-y-auto bg-white border rounded-xl divide-y">
                      {searchResults.map(place => (
                        <li key={place.id} onClick={() => handleSelectPlace(place)} className="p-3 hover:bg-orange-50 cursor-pointer text-sm"><div className="font-bold">{place.place_name}</div><div className="text-xs text-slate-400">{place.address_name}</div></li>
                      ))}
                    </ul>
                  )}
                ...
                <div className="space-y-3 p-4 bg-orange-50 rounded-2xl border border-orange-100">
                  <input required className="w-full p-3 rounded-xl border text-base lg:text-sm" placeholder="식당 이름" value={newRes.name} onChange={e => setNewRes({...newRes, name: e.target.value})} />
                  <input required className="w-full p-3 rounded-xl border text-base lg:text-sm" placeholder="도로명 주소" value={newRes.address} onChange={e => setNewRes({...newRes, address: e.target.value})} />
                </div>
              )}
              <select className="w-full p-3 bg-slate-50 rounded-xl border text-base lg:text-sm" value={newRes.category} onChange={e => setNewRes({...newRes, category: e.target.value})}>
              ...
              <div className="pt-4 border-t space-y-3">
                ...
                <input required className="w-full p-3 bg-slate-50 rounded-xl border text-base lg:text-sm" placeholder="한 줄 평" value={newReview.comment} onChange={e => setNewReview({...newReview, comment: e.target.value})} />
                <input required className="w-full p-3 bg-slate-50 rounded-xl border text-base lg:text-sm" placeholder="닉네임" value={newReview.author} onChange={e => setNewReview({...newReview, author: e.target.value})} />
              </div>
            </div>
            <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-orange-500 text-white rounded-xl font-bold shadow-lg">{isSubmitting ? <Loader2 className="animate-spin m-auto" size={20}/> : "등록하기 🚀"}</button>
          </form>
        </div>
      )}
      ...
      {isReviewModalOpen && selectedRes && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <form onSubmit={handleReviewSubmit} className="bg-white rounded-3xl w-full max-w-sm p-6 space-y-4 shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center"><h2 className="text-xl font-black">리뷰 남기기</h2><X className="cursor-pointer" onClick={() => setIsReviewModalOpen(false)} /></div>
            <div className="flex-1 overflow-y-auto space-y-4 py-2 custom-scrollbar">
              <div className="flex gap-2 justify-center py-2">{[1, 2, 3, 4, 5].map(i => <Star key={i} size={36} className={`cursor-pointer ${i <= newReview.rating ? 'fill-orange-400 text-orange-400' : 'text-slate-200'}`} onClick={() => setNewReview({...newReview, rating: i})} />)}</div>
              <textarea required className="w-full p-3 bg-slate-50 rounded-xl border h-32 text-base lg:text-sm" placeholder="어떠셨나요?" value={newReview.comment} onChange={e => setNewReview({...newReview, comment: e.target.value})}></textarea>
              <input required className="w-full p-3 bg-slate-50 rounded-xl border text-base lg:text-sm" placeholder="닉네임" value={newReview.author} onChange={e => setNewReview({...newReview, author: e.target.value})} />
            </div>
            <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-orange-500 text-white rounded-xl font-bold">{isSubmitting ? <Loader2 className="animate-spin m-auto" size={18}/> : "등록하기"}</button>
          </form>
        </div>
      )}
