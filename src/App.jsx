...
const App = () => {
  ...
  const [isSortByRating, setIsSortByRating] = useState(false); // 🌟 별점순 정렬 상태
  const [isCuisineOpen, setIsCuisineOpen] = useState(false); // 🌟 요리종류 드롭다운 상태
  ...
  const cuisineRef = useRef(null); // 드롭다운 외부클릭 감지용

  const CHURCH_ADDRESS = "서울 강북구 노해로 50";
  const CUISINES = ['한식', '중식', '일식', '양식', '분식']; // 🌟 묶을 요리 종류
  ...
  const filteredList = restaurants
    ...
    .sort((a, b) => {
      ...
      return 0; // 기본은 서버 데이터 순서 (혹은 추후 거리순)
    });
            ...
            <div className="flex items-center gap-2 mt-3 relative">
              ...
              <div className="relative" ref={cuisineRef}>
                <button onClick={() => setIsCuisineOpen(!isCuisineOpen)} className={`flex items-center gap-1 px-4 py-1.5 rounded-full text-xs font-bold border transition-all ${CUISINES.includes(activeCategory) ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600'}`}>
                  {CUISINES.includes(activeCategory) ? activeCategory : '요리 종류/분류'} <ChevronDown size={14} className={`transition-transform ${isCuisineOpen ? 'rotate-180' : ''}`} />
                </button>
                {isCuisineOpen && (
