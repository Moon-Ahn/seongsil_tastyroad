          ...
          <form onSubmit={handleAddRestaurant} className="bg-white rounded-[40px] w-full max-w-md flex flex-col shadow-2xl" style={{ maxHeight: '90vh' }}>
            ...
            <div className="overflow-y-auto flex-1 px-7 py-6 space-y-6 no-scrollbar">
              ...
              <div className="space-y-4">
                 <div><label className="text-[11px] font-black text-slate-400 ml-1">카테고리</label><select className="w-full mt-2 p-4 bg-slate-50 rounded-2xl outline-none text-base lg:text-sm focus:ring-2 focus:ring-orange-400" value={newRes.category} onChange={e => setNewRes({...newRes, category: e.target.value})}>{CATEGORIES.filter(c => c !== '전체').map(c => <option key={c}>{c}</option>)}</select></div>
                 <div className="pt-4 border-t border-slate-100 space-y-5">
                    <p className="text-sm font-black text-slate-700 text-center">첫 리뷰 남기기 ✨</p>
                    ...
                    <input required className="w-full p-4 bg-slate-50 rounded-2xl outline-none text-base lg:text-sm focus:ring-2 focus:ring-orange-400" placeholder="맛집 한 줄 평" value={newReview.comment} onChange={e => setNewReview({...newReview, comment: e.target.value})} />
            ...
            <div className="p-7 border-t shrink-0 bg-white"><button type="submit" disabled={isSubmitting} className="w-full py-4.5 bg-orange-500 text-white rounded-[24px] font-black shadow-xl shadow-orange-200 active:scale-95 transition-all flex justify-center items-center gap-2">{isSubmitting ? <Loader2 className="animate-spin" size={20}/> : <>맛집 등록 🚀</>}</button></div>
          </form>
        </div>
      )}
