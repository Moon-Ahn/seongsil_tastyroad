...
const App = () => {
  ...
  const handleDragRef = useRef({ active: false, startY: 0, startSheetY: sheetY });
  ...
  useEffect(() => { if (selectedRes && isMobile) setSheetY(60); }, [selectedRes, isMobile]);

  const onTouchStart = (e) => {
    if (!isMobile) return;
    const isHandle = e.target.closest('.sheet-handle-area');
    if (isHandle) handleDragRef.current = { active: true, startY: e.touches[0].clientY, startSheetY: sheetY };
  };
    ...
    const loadData = async () => {
      try {
        ...
        const grouped = data.reduce((acc, curr) => {
          if (!acc[curr.restaurant]) acc[curr.restaurant] = { id: curr.restaurant, name: curr.restaurant, address: curr.address || "", category: curr.category || "기타", isLunch: curr.isLunch === 'o', isVisitation: curr.isVisitation === 'o', reviews: [], avgRating: 0 };
      ...
      restaurants.forEach(res => {
        getCoordinates(res.address, (loc) => {
          ...
          const marker = new window.google.maps.Marker({
            ...
            icon: getPinIcon(themeColor), // 카테고리별 핀 색상 반영
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col lg:flex-row bg-white font-sans">
      ...
      <div className="relative flex-1 h-1/2 lg:h-full order-1 lg:order-2">
        ...
        <div className="absolute top-4 left-4 right-4 z-10 flex flex-col gap-2 pointer-events-none">
          ...
          <div className="flex gap-2 pointer-events-auto">
            <button onClick={() => setActiveCategory(activeCategory === '점심추천' ? '전체' : '점심추천')} className={`flex items-center gap-1.5 px-4 py-2.5 rounded-2xl shadow-lg font-bold text-[11px] transition-all active:scale-95 ${activeCategory === '점심추천' ? 'bg-slate-800 text-white' : 'bg-white text-slate-800 border border-slate-100'}`}><Sun size={14} /> 점심먹기 좋은 곳</button>
            <button onClick={() => setActiveCategory(activeCategory === '심방추천' ? '전체' : '심방추천')} className={`flex items-center gap-1.5 px-4 py-2.5 rounded-2xl shadow-lg font-bold text-[11px] transition-all active:scale-95 ${activeCategory === '심방추천' ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 border border-blue-100'}`}><Heart size={14} /> 심방하기 좋은 곳</button>
          </div>
        </div>
      ...
      <section className={`z-20 bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.12)] flex flex-col transition-all duration-300 ${isMobile ? 'fixed left-0 right-0 bottom-0 rounded-t-[32px]' : 'relative w-[400px] h-full shadow-none border-r border-slate-100'}`} style={isMobile ? { transform: `translateY(${sheetY}px)`, height: 'calc(100vh - 60px)', transition: handleDragRef.current.active ? 'none' : 'transform 0.3s ease-out' } : {}} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
        ...
        <div className={`flex-1 flex flex-col overflow-hidden ${selectedRes && isMobile ? 'hidden' : 'flex'}`}>
          ...
          <div ref={listRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-2.5 bg-white" onTouchStart={blockMap} style={{ overscrollBehavior: 'contain' }}>
            {activeCategory === '전체' && <button onClick={() => setSelectedRes(null)} className="w-full text-left p-4 rounded-3xl bg-blue-50 border border-blue-100 shadow-sm active:scale-95 transition-all"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-blue-100 rounded-2xl flex items-center justify-center shrink-0 shadow-inner"><Church size={18} className="text-blue-500" /></div><div><h3 className="font-bold text-blue-900 text-sm">수유 성실교회</h3><p className="text-[11px] text-blue-400 font-medium">우리들의 베이스캠프 ⛪</p></div></div></button>}
            {filteredList.map(res => (
              <div key={res.id} onClick={() => setSelectedRes(res)} className={`p-4 rounded-3xl cursor-pointer border-2 transition-all active:scale-98 ${selectedRes?.id === res.id ? 'bg-orange-50 border-orange-400 shadow-md' : 'bg-white border-slate-50 shadow-sm hover:shadow-md'}`}>
                <div className="flex items-center justify-between mb-2"><h3 className="font-bold text-slate-900 text-sm truncate pr-2">{res.name}</h3><div className="flex gap-1">{res.isLunch && <span className="text-[9px] bg-slate-800 text-white px-1.5 py-0.5 rounded font-bold">점심</span>}{res.isVisitation && <span className="text-[9px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded font-bold">심방</span>}<span className={`text-[10px] px-2.5 py-1 rounded-full font-bold shrink-0 ${res.category === '카페' ? 'bg-orange-100 text-orange-600' : 'bg-slate-800 text-white'}`}>{res.category}</span></div></div>
          ...
          <form onSubmit={handleAddRestaurant} className="bg-white rounded-[40px] w-full max-w-md flex flex-col shadow-2xl" style={{ maxHeight: '90vh' }}>
            ...
            <div className="overflow-y-auto flex-1 px-7 py-6 space-y-6 no-scrollbar">
              ...
              <div className="space-y-4">
                 <div><select className="w-full mt-2 p-4 bg-slate-50 rounded-2xl outline-none text-base lg:text-sm focus:ring-2 focus:ring-orange-400" value={newRes.category} onChange={e => setNewRes({...newRes, category: e.target.value})}>{CATEGORIES.filter(c => c !== '전체').map(c => <option key={c}>{c}</option>)}</select></div>
                 <div className="pt-4 border-t border-slate-100 space-y-5 text-center">
                    <p className="text-sm font-black text-slate-700">첫 번째 리뷰 남기기 ✨</p>
            ...
            <div className="p-7 border-t shrink-0 bg-slate-50/50 rounded-b-[40px]"><button type="submit" disabled={isSubmitting} className="w-full py-4.5 bg-orange-500 text-white rounded-[24px] font-black shadow-xl active:scale-95 transition-all flex justify-center items-center gap-2">{isSubmitting ? <Loader2 className="animate-spin" size={20}/> : "맛집 등록 완료 🚀"}</button></div>
          </form>
        </div>
      )}
      ...
      <style>{`
        * { -webkit-tap-highlight-color: transparent; }
        ...
        /* 🌟 박스 배경 제거 및 카테고리별 폰트 색상/크기 조정 (11px) */
        .restaurant-marker-label { color: #1E293B; font-size: 11px; font-weight: 800; text-shadow: -1px -1px 0 #FFF, 1px -1px 0 #FFF, -1px 1px 0 #FFF, 1px 1px 0 #FFF; white-space: nowrap; pointer-events: none; }
        .cafe-marker-label { color: #F97316; font-size: 11px; font-weight: 800; text-shadow: -1px -1px 0 #FFF, 1px -1px 0 #FFF, -1px 1px 0 #FFF, 1px 1px 0 #FFF; white-space: nowrap; pointer-events: none; }
        .church-marker-label { color: #1E3A8A; font-size: 11px; font-weight: 800; text-shadow: -1px -1px 0 #FFF, 1px -1px 0 #FFF, -1px 1px 0 #FFF, 1px 1px 0 #FFF; white-space: nowrap; pointer-events: none; }
