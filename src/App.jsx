...
const App = () => {
  ...
  // 🌟 위치 멀어지는 현상 방지를 위해 labelOrigin을 (12, 0)으로 고정
  ...
  useEffect(() => {
    ...
    getCoordinates(CHURCH_ADDRESS, (churchLoc) => {
      ...
      new window.google.maps.Marker({
        ...
        icon: getPinIcon('#FB7185'), label: { text: "수유 성실교회", className: "church-marker-label" },
        zIndex: 9999
      });
      restaurants.forEach(res => {
        getCoordinates(res.address, (loc) => {
          ...
          const labelText = `${res.name}${res.isCertified ? ' 🏅' : ''} ★${res.avgRating}`;
          const marker = new window.google.maps.Marker({
            map: mapInstance.current, position: loc, title: res.name, icon: getPinIcon(themeColor),
            label: { text: labelText, className: labelClass }, zIndex: 1
          });
  ...
  return (
    <div className="h-screen h-[100dvh] w-screen overflow-hidden flex flex-col lg:flex-row bg-white font-sans">
      ...
      <div className="relative flex-1 h-1/2 lg:h-full order-1 lg:order-2 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 right-4 z-10 flex flex-col gap-2 pointer-events-none">
          ...
          <div className="flex gap-2 pointer-events-auto overflow-x-auto no-scrollbar pb-1">
            <button onClick={() => setActiveCategory(activeCategory === '점심추천' ? '전체' : '점심추천')} className={`flex items-center gap-1.5 px-4 py-2.5 rounded-2xl shadow-lg font-bold text-[11px] whitespace-nowrap transition-all active:scale-95 ${activeCategory === '점심추천' ? 'bg-slate-800 text-white' : 'bg-white text-slate-800 border border-slate-100'}`}><Sun size={14} /> 점심 추천</button>
            <button onClick={() => setActiveCategory(activeCategory === '심방추천' ? '전체' : '심방추천')} className={`flex items-center gap-1.5 px-4 py-2.5 rounded-2xl shadow-lg font-bold text-[11px] whitespace-nowrap transition-all active:scale-95 ${activeCategory === '심방추천' ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 border border-blue-100'}`}><Heart size={14} /> 심방 추천</button>
      ...
      >
        ...
        <div className={`flex-1 flex flex-col overflow-hidden ${selectedRes && isMobile && isSheetExpanded ? 'hidden' : 'flex'}`}>
          <div className="px-4 pb-3 border-b border-slate-50 shrink-0 bg-white">
            ...
            <div className="flex items-center gap-2 mt-3 relative">
              ...
              <button onClick={() => setIsSortByRating(!isSortByRating)} className={`ml-auto flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${isSortByRating ? 'text-orange-500 bg-orange-50' : 'text-slate-400 bg-slate-50'}`}><Star size={14} className={isSortByRating ? 'fill-orange-400' : ''} /> 별점순</button>
            </div>
          </div>
        ...
        {selectedRes && isSheetExpanded && (
          <div className={`flex-1 flex flex-col overflow-hidden bg-white ${isMobile ? '' : 'absolute inset-0 z-30'}`}>
            <div className="px-5 pt-2 pb-5 border-b border-slate-50 shrink-0"><div className="flex justify-between items-start"><div className="flex-1 min-w-0 pr-4"><span className={`text-[11px] font-bold px-2.5 py-1 rounded-lg ${selectedRes.category === '카페' ? 'bg-orange-100 text-orange-600' : 'bg-slate-800 text-white'}`}>{selectedRes.category}</span><h2 className="text-2xl font-black text-slate-900 mt-2 leading-tight truncate">{selectedRes.name}{selectedRes.isCertified && ' 🏅'}</h2><div className="mt-3 space-y-1.5"><p className="text-xs text-slate-500 flex items-center gap-1.5 font-medium"><MapPin size={13} className="text-orange-400" />{selectedRes.address}</p>{distances[selectedRes.id] && <p className="text-xs font-bold text-blue-600 bg-blue-50 w-fit px-3 py-1 rounded-full shadow-sm flex items-center gap-1">⛪ 성실교회에서 {Math.round(distances[selectedRes.id])}m 거리</p>}</div></div><button onClick={() => setSelectedRes(null)} className="p-2.5 bg-slate-100 rounded-full active:scale-90 transition-all"><X size={20} className="text-slate-500" /></button></div><div className="flex gap-2.5 mt-5"><a href={`https://map.naver.com/v5/search/${encodeURIComponent(selectedRes.name + " 수유동")}`} target="_blank" rel="noreferrer" className="flex-1 bg-green-500 text-white py-3.5 rounded-2xl font-black text-sm flex items-center justify-center gap-2 shadow-lg active:scale-95 transition shadow-green-100"><MapIcon size={16} /> 길찾기</a><button onClick={() => setIsReviewModalOpen(true)} className="flex-1 bg-slate-900 text-white py-3.5 rounded-2xl font-black text-sm flex items-center justify-center gap-2 shadow-lg active:scale-95 transition shadow-slate-200"><MessageSquare size={16} /> 리뷰 쓰기</button></div></div>
      ...
      {/* 모달: 명예의 전당 */}
      {isRankModalOpen && (
        <div className="fixed inset-0 bg-slate-900/80 flex items-center justify-center z-[100] backdrop-blur-md p-4" onClick={() => setIsRankModalOpen(false)}><div className="bg-white rounded-[40px] w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}><div className="bg-gradient-to-br from-yellow-400 to-orange-500 p-8 text-center text-white relative"><button onClick={() => setIsRankModalOpen(false)} className="absolute top-4 right-4 bg-white/20 p-1.5 rounded-full hover:bg-white/30 transition-colors"><X size={20}/></button><Trophy size={48} className="mx-auto mb-3 drop-shadow-lg" /><h2 className="text-2xl font-black italic tracking-tighter">THANK YOU</h2><p className="text-sm font-bold opacity-90 mt-1">{new Date().getMonth() + 1}월의 맛집 전도사 TOP 3</p></div><div className="p-6 space-y-4">{Object.entries(restaurants.reduce((acc, res) => { res.reviews.forEach(rev => { const d = new Date(rev.timestamp); if (d.getMonth() === new Date().getMonth() && d.getFullYear() === new Date().getFullYear()) { acc[rev.author] = (acc[rev.author] || 0) + 1; } }); return acc; }, {})).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count).slice(0, 3).map((user, i) => (<div key={i} className={`flex items-center gap-4 p-5 rounded-3xl border-2 transition-all ${i === 0 ? 'bg-yellow-50 border-yellow-200 scale-105' : 'bg-slate-50 border-slate-100'}`}><div className="text-2xl">{i === 0 ? '🥇' : i === 1 ? '🥈' : '🥉'}</div><span className="flex-1 font-black text-slate-800 text-lg">{user.name}</span><span className="font-bold text-orange-600 bg-orange-100 px-4 py-1.5 rounded-full text-sm">{user.count}건</span></div>))}</div></div></div>
      )}
      ...
      <style>{`
        * { -webkit-tap-highlight-color: transparent; }
        ...
        .restaurant-marker-label { background-color: white; color: #1e293b; font-size: 11px; padding: 4px 10px; border-radius: 10px; border: 2.2px solid #1e293b; box-shadow: 0 4px 10px rgba(0,0,0,0.18); margin-top: -22px; white-space: nowrap; pointer-events: none; font-weight: 800; }
        .cafe-marker-label { background-color: white; color: #f97316; font-size: 11px; padding: 4px 10px; border-radius: 10px; border: 2.2px solid #f97316; box-shadow: 0 4px 10px rgba(249,115,22,0.18); margin-top: -22px; white-space: nowrap; pointer-events: none; font-weight: 800; }
        .church-marker-label { background-color: white; color: #FB7185; font-size: 11px; padding: 5px 12px; border-radius: 10px; border: 2.5px solid #FB7185; box-shadow: 0 4px 10px rgba(251,113,133,0.2); margin-top: -22px; white-space: nowrap; pointer-events: none; font-weight: 900; }
