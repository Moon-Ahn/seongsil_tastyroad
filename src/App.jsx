    ...
    <div className="h-screen w-screen overflow-hidden flex flex-col lg:flex-row bg-white font-sans">
      ...
      >
        ...
        {selectedRes && (
          <div className={`flex-1 flex flex-col overflow-hidden bg-white ${isMobile ? '' : 'absolute inset-0 z-30'}`}>
            ...
            <div ref={detailRef} className="flex-1 overflow-y-auto px-5 py-5 space-y-4 bg-white" onTouchStart={blockMap} style={{ overscrollBehavior: 'contain' }}>
              ...
              {selectedRes.reviews.length === 0 ? <p className="text-center text-slate-400 text-xs py-14 font-medium">아직 리뷰가 없어요. 첫 리뷰를 남겨주세요! ✨</p> :
                selectedRes.reviews.map((r, i) => (
                  <div key={i} className="bg-slate-50/50 p-5 rounded-[24px] shadow-sm border border-slate-100">
              ...
              {!isManualAddress ? (
                <div className="bg-slate-50 p-5 rounded-[28px] border border-slate-100 space-y-4 shadow-inner">
                  <div className="flex gap-2">
                    <input type="text" className="flex-1 p-3.5 bg-white rounded-2xl border-none shadow-sm outline-none text-base lg:text-sm focus:ring-2 focus:ring-orange-400" placeholder="식당 이름으로 검색" value={searchKeyword} onChange={e => setSearchKeyword(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), searchPlaces())} />
      ...
      <style>{`
        * { -webkit-tap-highlight-color: transparent; }
        ...
        /* 🌟 흰색 배경 박스 + 카테고리별 테두리 & 폰트색 (11px) */
        .restaurant-marker-label { background-color: white; color: #1e293b; font-size: 11px; padding: 4px 10px; border-radius: 10px; border: 2px solid #1e293b; box-shadow: 0 4px 10px rgba(0,0,0,0.15); margin-top: -32px; white-space: nowrap; pointer-events: none; font-weight: 800; }
        .cafe-marker-label { background-color: white; color: #f97316; font-size: 11px; padding: 4px 10px; border-radius: 10px; border: 2px solid #f97316; box-shadow: 0 4px 10px rgba(249,115,22,0.15); margin-top: -32px; white-space: nowrap; pointer-events: none; font-weight: 800; }
        .church-marker-label { background-color: white; color: #1e3a8a; font-size: 11px; padding: 5px 12px; border-radius: 10px; border: 2px solid #1e3a8a; box-shadow: 0 4px 10px rgba(0,0,0,0.15); margin-top: -32px; white-space: nowrap; pointer-events: none; font-weight: 900; }
